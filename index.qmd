...
While my sub-questions have changed throughout the course of working on this project, my overall question: *"Who are these environmental defenders as people"* remained the same. To answer this, I wanted to explore who they were both in their name, their personal background, and the circumstances surrounding their death. This exploration led to the sub-questions of *"What are the links between environmental defenders and the perpetrators of their deaths?"* and *"Out of the total number of victims, what person demographics experience violent attacks the most/ who are the primary organizations carrying out these attacks?"*. Using data sourced from [Global Witness](https://www.globalwitness.org/en/campaigns/environmental-activists/defending-tomorrow/), I used these questions to guide my exploration into the data and the production of the final visualizations. Since the dataset was so large and spanned over multiple countries, I chose to focus on Colombia. Colombia has experienced an increase in violent attacks against environmental defenders as tensions rise between the introduction of environmental movements and policy and the need to extract resources. In 2019 Colombia had experienced the highest rate of environmental defender deaths that were able to be recorded by Global Witness compared to every other year and country. 

Some limitations of the dataset and the visualizations are due to the nature of the dataset. Crimes are often underreported, misreported, or unreported all together, making it difficult to make any conclusions based on the data and visualizations alone. In places that experience high rates of oppresion against minority groups by police and military agencies and face corruption within these organizations, that makes getting accurate data and truly representative data even more difficult, both due to the data being obscured by these factors and in the protection of the identities of the victims and their families. Global Witness has vetted all information in the data set and only included cases that could be definitively proven. Even with their efforts, more information on victims themselves and the land industries/sectors that were the driving force behind their deaths is not available in detail, or much of that data is missing

I chose to do a text based graph as my first plot to introduce the viewer/myself to the victims. I felt that this was an important and necessary step when working with data covering a very sensitive subject matter, and in dealing with data involving people. Making sure that there was an awareness of the victims life and existence outside of this project and in honor of their sacrifices in protecting the environment was something that needed to be introduced first. I chose to do a sankey diagram as the second visualization to see where the connections lay between the demographics of the victims, and the different organizations responsible for their deaths, and what demographics and organizations had no connections between eachother. For my final plot, I chose a bar graph to put a numeric value to the connections made in the sankey diagram. 

I kept text minimal and gave the basic information necessary to understand the context for all three plots. For the sankey diagram, I had to get creative with the axis labels and the stratum labels since it's very difficult to adjust the stratum boxes in a way that makes the text easy to read. I colored the labels in the original plot white and individually added text annotations for each axis and stratum box to get custom positions and sizes for all labels.

The `theme()` function was used to modify the text font, size, face, and position for all plots as well as modifying plot margins where necessary and removing gridlines and axis ticks and all plots except the bar plot. I felt that the gridlines and ticks were only necessary on the bar plot since it was the only plot displaying counts of the data and it would help to better orient the viewer to where the plots fall in relation to the flipped y-axis. I also chose to do a coordinate flip on both the sankey and bar plots to make the long names of the axis labels more readable.

For the sankey diagram and bar plot, I created two custom color palettes using [coolors](https://coolors.co/) with a neutral but enaging palette. Since the the visualizations are on sensitive subject matters, I wanted to be respectful in my choice of color in not chosing something bright or distracting and went for a neutral, earth-toned palette. I chose these earth tones since the data is related to the land and the environment, and since the victims had dedicated their lives to protecting the environment it seemed fitting. I chose not to use colors other than black in the text based graph to keep the plot simple. I also included dandelion icons next to each of the victims name

I used the Cormorant Garamond and Proza Libre fonts for all plots based on the suggestion to pair these fonts together from [fontpair](https://www.fontpair.co/). For accessability, I included alt-text to each of the plots produced.

For the infographic, I wanted to keep the same plot hierarchy that i produced through code to guide the viewers through the same experience/questions. I included text boxes to provide more background on the data including the decision to focus on Colombia in 2019, as well as the continued oppression against minority groups as climate change worsens overtime. I also used the text boxes to provide more information on stand-out results from the bar plot.
...
#| fig-align: "center"
#| out-width: "100%"
knitr::include_graphics("outputs/text.png")
```
...
#| fig-align: "center"
#| out-width: "100%"
knitr::include_graphics("outputs/sanky.png")
```
...
#| fig-align: "center"
#| out-width: "100%"
knitr::include_graphics("outputs/bar.png")
```
